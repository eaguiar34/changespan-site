    ...
    <main className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <a href={getCorePackageExportCsvUrl(packageId)} className="rounded-2xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-900">
            Export CSV
          </a>
          <a href={getCorePackageExportJsonUrl(packageId)} className="rounded-2xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-900">
            Export JSON
          </a>
        </div>
      </div>
          ...
          <ul className="mt-4 space-y-2">
            {detail.attachments.map((a: any) => (
              <li key={a.id} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <a className="underline font-semibold text-slate-900" href={getCoreAttachmentDownloadUrl(a.id)}>
                  Download
                </a>
              </li>
            ))}
